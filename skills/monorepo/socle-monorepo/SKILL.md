...
Écrire aussi `docs/features/socle/socle.spec.md` : une page qui dit ce qu'est le socle, quels services sont installés, ce qu'il ne fait pas, et dans quel ordre attaquer les tickets — les siens **et** ceux des services. **L'ordre n'est pas par couche.** Après le ticket `1`, la suite est une **première balle traçante** — un use case réel de bout en bout, du contrat à l'écran — et les tickets de couche (`4`, `5`, sessions) se dimensionnent à ce qu'elle exige. Un ordre « base, puis authentification, puis sessions » avant qu'un écran ne montre une donnée métier est exactement l'anti-pattern que `conception.md` nomme.
